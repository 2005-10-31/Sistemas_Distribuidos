...
Em Microserviços, a verificação fica centralizada no Gateway, que é o único ponto de entrada externo. Os serviços internos (utilizadores, produtos, pedidos) não verificam o JWT, mas exigem um segredo partilhado (`X-Internal-Token`) que só o Gateway envia — por isso uma chamada direta às portas 5001/5002/5003 é recusada com **403**. O `/health` fica aberto para monitorização.

## Configuração de segredos (.env)

As chaves e credenciais **não** estão no código — ficam em ficheiros `.env`, que estão no `.gitignore` e nunca são enviados para o repositório. Antes de correr, copie o ficheiro de exemplo e preencha os valores:
...
| ---------------- | ---------------- | ----------------------------------------------- |
| `JWT_SECRET`     | Monolito + Micro | Chave para assinar/verificar os tokens JWT      |
| `JWT_EXPIRATION` | Monolito + Micro | Validade do token (ex.: `1h`)                   |
| `ADMIN_USERS`    | Monolito + Micro | Credenciais de login, em JSON **numa só linha** |
| `INTERNAL_TOKEN` | Microserviços    | Segredo partilhado Gateway ↔ serviços internos  |

### Credenciais de teste

As credenciais são definidas na variável `ADMIN_USERS` do `.env` (ver `.env.example`) — não estão no código nem no repositório.
