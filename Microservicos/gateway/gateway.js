...
if (!JWT_SECRET || !INTERNAL_TOKEN) {
  console.error(
    "ERRO: JWT_SECRET e INTERNAL_TOKEN devem estar definidos no .env",
  );
  process.exit(1);
}

const cabecalhoInterno = { headers: { "X-Internal-Token": INTERNAL_TOKEN } };
...
app.get("/:servico", verificarToken, async (req, res) => {
  ...
  try {
    ...
    const resposta = await axios.get(`${url}/${servico}`, cabecalhoInterno);
...
app.get("/:servico/:id", verificarToken, async (req, res) => {
  ...
  try {
    ...
    const resposta = await axios.get(`${url}/${servico}/${id}`, cabecalhoInterno);
    const instanciaId = resposta.headers["x-instance-id"];
    if (instanciaId) res.setHeader("X-Instance-ID", instanciaId);
    res.json(resposta.data);
  } catch (erro) {
...
app.post("/:servico", verificarToken, async (req, res) => {
  ...
  try {
    ...
    const resposta = await axios.post(`${url}/${servico}`, req.body, cabecalhoInterno);
    const instanciaId = resposta.headers["x-instance-id"];
    if (instanciaId) res.setHeader("X-Instance-ID", instanciaId);
